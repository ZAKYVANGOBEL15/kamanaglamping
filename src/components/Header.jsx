    ...
    <header className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 h-16 md:h-20 flex items-center justify-between">
        ...
        <nav className={`hidden md:flex items-center gap-8 text-xs font-semibold uppercase tracking-wider transition-colors duration-300 ${
          ...
          <a href="https://wa.me/6281234567890" target="_blank" rel="noopener noreferrer" className={`transition-colors duration-200 ${isAtTop ? 'hover:text-white' : 'hover:text-[#1b3327]'}`}>Kontak</a>
        </nav>
      </div>
    </header>
  );
